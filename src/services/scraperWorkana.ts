...
function parseRelativeDate(text: string): string {
  ...
  if (minutes)
    return formatPanamaDate(new Date(now.getTime() - parseInt(minutes) * ms));
  if (hours)
    return formatPanamaDate(new Date(now.getTime() - parseInt(hours) * hs));
  if (matchHourGeneric) return formatPanamaDate(new Date(now.getTime() - hs));
  if (days)
    return formatPanamaDate(new Date(now.getTime() - parseInt(days) * ds));
  if (matchYesterday) return formatPanamaDate(new Date(now.getTime() - ds));
  if (weeks)
    return formatPanamaDate(new Date(now.getTime() - parseInt(weeks) * 7 * ds));
  ...
  return await page.evaluate(() => {
    ...
    if (!projectCards || projectCards.length === 0) {
      projectCards = document.querySelectorAll(
        'article, .card, [class*="project"], [class*="job"]',
      );
    }
...
async function getPageCount(page: Page): Promise<number[]> {
  return await page.evaluate(() => {
    ...
    const paginationItems = document.querySelectorAll(
      '.pagination a, [class*="page-number"], [data-page]',
    );
